...
반출판(denamer_out)과 질의판(denamer_ask)이 이 모듈 하나를 공유한다.
...
RULES: list[Rule] = [
    ...
    _r("ACCOUNT", rf"(?<![\d\-−–—―])\d{{2,6}}(?:{SP}*{DASH}{SP}*\d{{2,6}}){{1,3}}(?![\d\-−–—―])"
                  rf"|(?<!\d)\d{{10,16}}(?!\d)",
    ...
    _r("CASE", rf"{BL}\d{{4}}{SP}*(?:{_COURT_CODES}){SP}*\d+{BR}"),
    # 2자리 연도형은 차량번호(81나3166)와 형태가 같다 → 사건 문맥일 때만
    _r("CASE", rf"{BL}\d{{2}}{SP}*(?:{_COURT_CODES_SHORT}){SP}*\d{{2,6}}{BR}",
       context=CASE_CONTEXT),
    _r("CASE", rf"{BL}(?:20)?\d{{2}}{SP}*(?:형제|전형제|중제|공제|징수){SP}*\d+호?{BR}"),
    _r("CASE", rf"{BL}(?:서울|부산|인천|경기|강원|충북|충남|전북|전남|경북|경남|제주|중노위)?"
               rf"{SP}*\d{{4}}{SP}*(?:부해|부노|부차|부당){SP}*\d+{BR}"),
    _r("CASE", rf"{BL}조심{SP}*\d{{4}}{SP}*[가-힣]?{SP}*\d{{3,5}}{BR}"),
    _r("CASE", rf"{BL}(?:중앙행심|행심){SP}*\d{{4}}[-–]?\d{{1,6}}{BR}"),
    ...
    _r("ADDR_DETAIL", r"(?P<v>\([가-힣0-9\s,·.-]{2,30}(?:동|가|리|로|길)[가-힣0-9\s,·.-]{0,30}\))"),
