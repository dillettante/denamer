...
- `ko-pii` 미설치 시 정규식·규칙 탐지만으로 동작합니다. 이때 **라벨 없는 자유문장 속
  이름은 탐지 범위 밖**이며, 그 사실이 리포트의 `engine.ko_pii`와 경고로 표시됩니다.
  테스트도 구성을 알아보고 해당 항목을 skip 처리하므로 두 구성 모두 통과해야 정상입니다.
- 종료 코드
  - **0** 정상
  - **1** 잔존·매핑실패 — 출력물을 신뢰하지 마세요
  - **2** 경고 있음 — 산출물은 생성됐지만 축소 모드로 돌았거나 출력물에 PII 모양이
    남아 있습니다. `warnings`를 읽고 판단하세요.
...
6. **독립 모양 점검** — 위 4·5는 *자기가 탐지한 값*만 다시 찾습니다. 미탐은 검증 대상에
   애초에 들어가지 않으므로 원리적으로 걸리지 않습니다(리포트가 깨끗한 것과 문서가
   깨끗한 것은 다른 말입니다). 그래서 탐지 규칙을 재사용하지 않는 훨씬 느슨한 패턴
   (주민번호·사건번호·전화·번호열 모양)을 출력물에 따로 돌려 `warnings`로 올립니다.
   오탐이 섞이는 것이 정상이며, 목적은 **"0건 = 안전"과 "0건 = 규칙이 못 봤음"을
   구분**하는 것입니다. 경고에는 원문 대신 마스킹된 표본만 싣습니다.
